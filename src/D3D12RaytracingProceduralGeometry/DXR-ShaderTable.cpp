	...
	auto GetShaderIDs = [&](auto* stateObjectProperties)
	{
		...
		for (UINT i = 0; i < IntersectionShaderType::Count; i++) {
			for (UINT j = 0; j < RayType::Count; j++) {
				hitGroupShaderIDs_AABBGeometry[i][j] =
					stateObjectProperties->GetShaderIdentifier(c_hitGroupNames_AABBGeometry[i][j]);
				shaderIdToStringMap[hitGroupShaderIDs_AABBGeometry[i][j]]
					= c_hitGroupNames_AABBGeometry[i][j];
			}
		}
	};
