...
void DXProceduralProject::BuildBottomLevelASInstanceDescs(BLASPtrType *bottomLevelASaddresses, ComPtr<ID3D12Resource>* instanceDescsResource)
{
	...
	// * Make each instance hover above the ground by ~ half its width
	{
		...
		// Scale in XZ dimensions. (POTENTIAL BUG)
		XMMATRIX mScale = XMMatrixScaling(c_aabbWidth, c_aabbWidth, c_aabbWidth);
		XMMATRIX mTranslation = XMMatrixTranslation(0, c_aabbWidth * 0.5f, 0.f);
		XMMATRIX mTransform = mScale * mTranslation;
...
AccelerationStructureBuffers DXProceduralProject::BuildTopLevelAS(AccelerationStructureBuffers bottomLevelAS[BottomLevelASType::Count], D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags)
{
	...
	if (m_raytracingAPI == RaytracingAPI::FallbackLayer)
	{
		...
		BuildBottomLevelASInstanceDescs< D3D12_RAYTRACING_FALLBACK_INSTANCE_DESC, WRAPPED_GPU_POINTER>
	...
	else // DirectX Raytracing
	{
		...
		BuildBottomLevelASInstanceDescs< D3D12_RAYTRACING_INSTANCE_DESC, D3D12_GPU_VIRTUAL_ADDRESS>
		(bottomLevelASaddresses, &instanceDescsResource);	
	}
...
void DXProceduralProject::BuildAccelerationStructures()
{
	...
	for (int i = 0; i < BottomLevelASType::Count; ++i) {
		bottomLevelAS[i] = BuildBottomLevelAS(geometryDescs[i], D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE);
	}
	...
	AccelerationStructureBuffers topLevelAS = BuildTopLevelAS(bottomLevelAS, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE);
