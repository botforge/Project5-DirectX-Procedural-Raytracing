...
void DXProceduralProject::CreateAABBPrimitiveAttributesBuffers()
{
	...
	m_aabbPrimitiveAttributeBuffer.Create(device, num_ob, 3, L"AABB Primitive Structured Buffer");
}
...
void DXProceduralProject::UpdateAABBPrimitiveAttributes(float animationTime)
{
	...
	auto SetTransformForAABB = [&](UINT primitiveIndex, XMMATRIX& mScale, XMMATRIX& mRotation)
	{
		...
		auto transform = XMMatrixMultiply(XMMatrixMultiply(mScale, mRotation), mTranslation);
		m_aabbPrimitiveAttributeBuffer[primitiveIndex].localSpaceToBottomLevelAS = transform;
		auto invTransform = XMMatrixInverse(nullptr, transform);
		m_aabbPrimitiveAttributeBuffer[primitiveIndex].bottomLevelASToLocalSpace = invTransform;
	};
